...
export function calculateGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const basePower = calculateBasePowerGLACE(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackGLACE(gen, attacker, defender, move, field, desc, isCritical);
  // #endregion

  // #region (Special) Defense

  const defense = calculateDefenseGLACE(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageGLACE(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const finalMods = calculateFinalModsGLACE(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateGLACE(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackGLACE(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseGLACE(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerGLACE(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageGLACE(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsGLACE(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsGLACE(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateAttackGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsGLACE(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateDefenseGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsGLACE(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamageGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsGLACE(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
