...
export function calculateTM(
  ...
  const basePower = calculateBasePowerTM(
    gen,
    attacker,
    defender,
    move,
  ...
  const attack = calculateAttackTM(gen, attacker, defender, move, field, desc, isCritical);
  // #endregion

  // #region (Special) Defense

  const defense = calculateDefenseTM(gen, attacker, defender, move, field, desc, isCritical);
  ...
  const baseDamage = calculateBaseDamageTM(
    gen,
    attacker,
    defender,
    basePower,
  ...
  const finalMods = calculateFinalModsTM(
    gen,
    attacker,
    defender,
    move,
  ...
  if (attacker.hasAbility('Parental Bond') && move.hits === 1 && !isSpread) {
    ...
    childDamage = calculateTM(gen, child, defender, move, field).damage as number[];
    desc.attackerAbility = attacker.ability;
  }
  ...
  if (move.timesUsed! > 1 || move.hits > 1) {
    ...
    for (let times = 1; times < numAttacks; times++) {
      ...
      const newAttack = calculateAttackTM(gen, attacker, defender, move,
        field, desc, isCritical);
      const newDefense = calculateDefenseTM(gen, attacker, defender, move,
        field, desc, isCritical);
      ...
      const newBasePower = calculateBasePowerTM(
        gen,
        attacker,
        defender,
        move,
      ...
      const newBaseDamage = calculateBaseDamageTM(
        gen,
        attacker,
        defender,
        newBasePower,
      ...
      const newFinalMods = calculateFinalModsTM(
        gen,
        attacker,
        defender,
        move,
...
export function calculateBasePowerTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const bpMods = calculateBPModsTM(
    gen,
    attacker,
    defender,
    move,
...
export function calculateBPModsTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
export function calculateAttackTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const atMods = calculateAtModsTM(gen, attacker, defender, move, field, desc);
  attack = OF16(Math.max(1, pokeRound((attack * chainMods(atMods, 410, 131072)) / 4096)));
  return attack;
}

export function calculateAtModsTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  if (defender.teraType && move.isMax) {
    atMods.push(3072);
  } else if (defender.isDynamaxed && move.named('Tera Blast')) {
    atMods.push(6144);
  }
...
export function calculateDefenseTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
  ...
  const dfMods = calculateDfModsTM(
    gen,
    attacker,
    defender,
    move,
...
export function calculateDfModsTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
...
function calculateBaseDamageTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  basePower: number,
...
export function calculateFinalModsTM(
  gen: Generation,
  attacker: Pokemon,
  defender: Pokemon,
  move: Move,
