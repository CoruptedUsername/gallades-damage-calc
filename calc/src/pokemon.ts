...
export class Pokemon implements State.Pokemon {
  ...
  maxHP(original = false) {
    ...
    if (!original && this.gen.num === 27 && this.isDynamaxed === 'gmax' &&
      this.species.baseStats.hp !== 1) {
      return Math.floor((this.rawStats.hp * (125 + 2.5 * this.dynamaxLevel!)) / 100);
    } else if (!original && this.isDynamaxed && this.species.baseStats.hp !== 1) {
